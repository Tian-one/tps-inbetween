...
def match2flow(linematch0, linematch1, h, w):
    assert len(linematch0.shape) == 3 and linematch0.shape[1] == 2 and linematch0.shape[2] == 2
    assert len(linematch1.shape) == 3 and linematch1.shape[1] == 2 and linematch1.shape[2] == 2
    assert linematch0.shape[0] == linematch1.shape[0]

    flow_x = torch.zeros((h, w))
    flow_y = torch.zeros((h, w))

    for line_idx in range(linematch0.shape[0]):
        move = linematch1[line_idx] - linematch0[line_idx]
        print(move)
...
def main():
    ...
    parser.add_argument('-img1', default='/home/zty/datasets/anime_sketch_test/shigatsu/cut13/000032.png')
    parser.add_argument('-img2', default='/home/zty/datasets/anime_sketch_test/shigatsu/cut13/000033.png')
    ...
    match2flow(matched_lines0, matched_lines1, h=288, w=512)
